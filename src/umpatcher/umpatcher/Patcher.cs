...
namespace UnityMonoDllSourceCodePatcher {
	abstract class Patcher {
		...
		void CopyOriginalUnityFiles() {
			...
			Log($"Checking out unity repo, commit hash: {unityGitHash}");
			unityRepo.CheckOut(unityGitHash, force: true);

			//dnSpyRepo.CheckOut(Constants.DnSpyUnityRepo_master_Branch);
			//dnSpyRepo.ThrowIfTreeNotClean();

			Log($"Checking out external submodules");
			...
			TextFilePatcher.RemoveLines(gitignore, line => line.Text == "/opcode.def");
